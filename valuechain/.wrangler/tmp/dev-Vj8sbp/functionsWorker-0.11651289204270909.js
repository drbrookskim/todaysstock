...
// .wrangler/tmp/pages-nlKMgI/functionsWorker-0.11651289204270909.mjs
var __defProp2 = Object.defineProperty;
var __name2 = /* @__PURE__ */ __name((target, value) => __defProp2(target, "name", { value, configurable: true }), "__name");
var TopicAnalyzer = class {
  static {
    __name(this, "TopicAnalyzer");
  }
  static {
    __name2(this, "TopicAnalyzer");
  }
  constructor(nTopics = 5) {
  ...
  generateFromKeyword(keyword) {
    ...
    let relatedTopics = [];
    for (const [key, topics] of Object.entries(this.knowledgeBase)) {
      if (keywordLower.includes(key)) {
        const count = Math.floor(Math.random() * (6 - 3)) + 3;
        relatedTopics = this._shuffle(topics).slice(0, count);
        break;
      }
    }
    if (relatedTopics.length === 0) {
      const baseTopics = ["Impact", "Trends", "Tools", "Future", "Community", "Research", "Education"];
      const count = Math.floor(Math.random() * (5 - 3)) + 3;
      relatedTopics = this._shuffle(baseTopics).slice(0, count).map((t) => `${keyword} ${t}`);
    }
    const analysis = { topics: {}, keywords: [], stocks: [] };
    let domain = null;
    ...
    if (domain) {
      const stocksCount = Math.floor(Math.random() * (4 - 2)) + 2;
      analysis.stocks = this._shuffle(this.tickerMap[domain]).slice(0, stocksCount);
    }
    relatedTopics.forEach((topicName) => {
      analysis.topics[topicName] = {
        percentage: 100 / relatedTopics.length,
        message_count: 1
      };
      const kwCount = Math.floor(Math.random() * (4 - 2)) + 2;
      for (let j = 0; j < kwCount; j++) {
        const variations = ["Core", "Advanced", "History", "Best Practice", "Framework", "System"];
        const kwWord = `${topicName.split(" ")[0]} ${variations[Math.floor(Math.random() * variations.length)]}`;
        const score = Math.random() * (0.4 - 0.1) + 0.1;
        analysis.keywords.push([kwWord, score]);
      }
    });
    return analysis;
  }
...
async function onRequestPost({ request }) {
  try {
    ...
    topicEntries.forEach(([topicName, data2], i) => {
...
function constructGraph(analysis, rootLabel = "Analysis Graph") {
  const nodes = [];
  const links = [];
  nodes.push({
    id: "root",
  ...
  topicEntries.forEach(([topicName, data], i) => {
    ...
    nodes.push({
      ...
      type: "topic",
      val: 3,
      description: `This topic represents ${data.percentage.toFixed(1)}% of the analysis.`
    });
    links.push({ source: "root", target: topicId });
    const keywordsPerTopic = 3;
    const startIdx = i * keywordsPerTopic;
    const topicKeywords = allKeywords.slice(startIdx, startIdx + keywordsPerTopic);
    topicKeywords.forEach(([word, score], kwIdx) => {
      const kwId = `kw_${i}_${kwIdx}`;
      let generatedContent = `### ${word.charAt(0).toUpperCase() + word.slice(1)}

`;
      generatedContent += `This keyword was identified with a relevance score of ${score.toFixed(4)}.

`;
      generatedContent += `In the current context, **${word}** is a key theme. `;
      generatedContent += `It relates closely to the topic '${topicName}'.

`;
      generatedContent += `#### Key Insights:
`;
      generatedContent += `- Relevant to thematic clusters.
`;
      generatedContent += `- Strong correlation with neighboring nodes.
`;
      generatedContent += `- Actionable item: Explore more about this specific area.`;
      nodes.push({
        id: kwId,
        label: word,
        type: "keyword",
...
This stock is highly relevant to the main keyword **${rootLabel}**.

#### Market Insights:
- Sector: Technology/Core domain
- Relevance: Direct investment opportunity related to '${rootLabel}'.

[View Real-time Chart (External)](https://finance.yahoo.com/quote/${ticker})`
    });
    links.push({ source: "root", target: stockId });
  });
  return { nodes, links };
}
__name(constructGraph, "constructGraph");
__name2(constructGraph, "constructGraph");
...
// .wrangler/tmp/bundle-lJzpxH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-lJzpxH/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.11651289204270909.js.map
