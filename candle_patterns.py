        ...
        if abs(v1 - v2) / max(v1, v2) < 0.08:
            if current_close < v2 * 1.03: 
                return {
                    ...
                    "signal": "bearish",
                    "description": "현재 뚜렷한 확정 패턴은 없으나, [이중 천장형(쌍봉)] 패턴이 형성될 가능성이 관측되고 있습니다. 고점 저항 돌파 여부를 주의 깊게 관찰하세요.",
                    "confidence": 0.45,
                    "type": "reversal"
                }
...
def analyze_candle_patterns(df):
    ...
    atr_data = da.compute_atr_targets(df)
    prob_data = da.compute_trade_probability(df, detected)
    vol_anomaly = da.detect_volume_anomaly(df)
    cycle_data = da.compute_cycle_estimation(df)

    # ── 매수/매도 시그널 상호 배제 (Mutual Exclusion) 필터링 ──
    ...
    if bullish_patterns and bearish_patterns:
        best_buy_conf = max(p["confidence"] for p in bullish_patterns)
        best_sell_conf = max(p["confidence"] for p in bearish_patterns)
        
        if best_buy_conf > best_sell_conf:
            bearish_patterns = []  # 매수 시그널이 더 강력하므로 매도 무시
        elif best_sell_conf > best_buy_conf:
            bullish_patterns = []  # 매도 시그널이 더 강력하므로 매수 무시
        else:
            # 신뢰도가 완벽히 동일하다면 종합 트렌드 점수(Total Score)로 판가름
            if bullish_score > bearish_score:
                bearish_patterns = []
            elif bearish_score > bullish_score:
                bullish_patterns = []
            # 둘 다 완전히 동일하면(매우 희박), 정보 제공 차원에서 둘 다 남김
    ...
    if bullish_patterns:
        ...
        if atr_data:
            aggressive_entry = atr_data["current"]
            conservative_entry = round(atr_data["current"] * 0.98)
            target_price = atr_data["target"]
            stop_loss = atr_data["stop_loss"]
        else:
            aggressive_entry = int(current_price)
            conservative_entry = int(current_price * 0.98)
            target_price = int(current_price * 1.1)
        ...
        buy_report = {
            ...
            "entry_tip": "전일 종가 부근에서 지지 확인 후 진입 추천"
        }
    else:
        buy_report = None
    ...
    if bearish_patterns:
        ...
        sell_report = {
            ...
            "risk_reward": "1:2.0",
            "volume_note": volume_note,
            "exit_tip": "분할 매도로 대응하며 추가 하락 지지선 확인 권장"
        }
    else:
        sell_report = None
    ...
    return {
        ...
        "volume_note": volume_note,
        "atr_targets": atr_data,
        "trade_probability": prob_data,
        "volume_anomaly": vol_anomaly,
        "cycle_estimation": cycle_data,
    }
